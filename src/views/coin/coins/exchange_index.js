...
export class ExchangeIndex extends React.Component {
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: "交易所",width: 100, dataIndex: "market_name", sorter: true, render(value, record) {
        return <span><Link to={"/manage/coin/markets/"+record['market_code']}>{record['market_name']}</Link> #{record['market_rank']!=9999 ? record['market_rank'] : "无"}</span>
      }
      },
      {
