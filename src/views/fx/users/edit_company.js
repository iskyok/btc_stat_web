...
import {getUser, getUsers, saveUser, updateUser, moveRelationToCompany} from '../../../reducers/actions/fx/users';
...
import SearchCompany from "../../shared/search_company"
...
let MyForm = React.createClass({
  ...
  handleSubmit() {
    ...
    form.validateFields((errors) => {
      ...
      let company_id = this.refs.searchCompany.state.selectValue;
      console.log("uuuu---------", this.refs.searchCompany.state)
      if (!!!company_id) {
        message.error("请选择上级目标用户")
        return false
      }
      params["company_id"] = company_id;
      params["move_all"] = true;
      if (this.props.itemId) {
        this.props.moveRelationToCompany(this.props.itemId, params)
      }
    });
  },
  // 只展示最后一项
      ...
      <Modal title={this.props.title} visible={true} onOk={this.handleSubmit.bind(this)}
        ...
        <Form horizontal form={this.props.form}>
          ...
          <FormItem {...formItemLayout} label="分公司">
            <div>
              <SearchCompany ref="searchCompany"></SearchCompany>
            </div>
          </FormItem>
...
function mapDispatchToProps(dispatch, ownProps) {
  return {
    ...
    }, moveRelationToCompany: (id, params) => {
      dispatch(moveRelationToCompany(id, params)).then(() => {
        ownProps.closeForm()
