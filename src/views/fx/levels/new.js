...
import {getLevel, getlevels, saveLevel, updateLevel} from '../../../reducers/actions/fx/levels';
  ...
  render(){
    ...
    return (<Form horizontal form={this.props.form}>
      ...
      <FormItem {...formItemLayout} label="名称">
        <Input {...getFieldProps("level.name", requiredConf())} type="text" autoComplete="off" disabled={this.state.Itemvisible}/>
      </FormItem>

      <FormItem {...formItemLayout} label="等级">
        <Input {...getFieldProps("level.sort", requiredConf())} type="text" autoComplete="off" disabled={this.state.Itemvisible}/>
      </FormItem>

      <FormItem {...formItemLayout} label="一级分销获利">
        <Input {...getFieldProps("level.dealer1_percent", requiredConf())} type="text" autoComplete="off" disabled={this.state.Itemvisible}/>
      </FormItem>

      <FormItem {...formItemLayout} label="二级分销获利">
        <Input {...getFieldProps("level.dealer2_percent", requiredConf())} type="text" autoComplete="off" disabled={this.state.Itemvisible}/>
      </FormItem>
      {console.log("jjjj",this.props.itemData)}
...
function mapDispatchToProps(dispatch) {
  return {
    getItemData: (params) => dispatch(getLevel(params)),
    saveItemData: (params)=>{
       dispatch(saveLevel(params)).then(()=> {
         dispatch(getLevels())
       })
    },
    updateItemData: (id, params) => dispatch(updateLevel(id, params))
  }
}
MyForm = createForm({
    ...
    let result = {};
        if ( props.formType === "newForm"){
          ...
          console.log(props,result , "props result ===============")
            for ( let prop1 in props.itemData ) {
              result["level." + prop1] = {value: props.itemData[prop1]}
             }
       return result;
        }
  }
...
let LevelNew = React.createClass({
  getInitialState() {
