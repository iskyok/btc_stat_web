...
import {getApply, getapplies, saveApply, updateApply} from '../../../reducers/actions/fx/applies';
  ...
  render(){
    ...
    return (<Form horizontal form={this.props.form}>
      ...
      <FormItem {...formItemLayout} label="名称">
        <Input {...getFieldProps("apply.name", requiredConf())} type="text" autoComplete="off"/>
      </FormItem>

      <FormItem {...formItemLayout} label="手机">
        <Input {...getFieldProps("apply.account_phone", requiredConf())} type="text" autoComplete="off"/>
      </FormItem>

      <FormItem {...formItemLayout} label="等级">
        <Input {...getFieldProps("apply.level_name", requiredConf())} type="text" autoComplete="off"/>
      </FormItem>

      <FormItem {...formItemLayout} label="所属上级分销商，个人消费">
        <Input {...getFieldProps("apply.prev_dealer_name", requiredConf())} type="text" autoComplete="off"/>
      </FormItem>

      <FormItem {...formItemLayout} label="状态">
        <Input {...getFieldProps("apply.state_name", requiredConf())} type="text" autoComplete="off"/>
      </FormItem>

      <FormItem {...formItemLayout} label="创建时间">
        <Input {...getFieldProps("apply.created_at", requiredConf())} type="text" autoComplete="off"/>
      </FormItem>
...
function formMapStateToProps(state) {
  return {
    itemData: state.fx_applies.itemData
  };
}
//redux动作
function mapDispatchToProps(dispatch) {
  return {
    getItemData: (params) => dispatch(getApply(params)),
    saveItemData: (params)=>{
       dispatch(saveApply(params)).then(()=> {
         dispatch(getApplies())
       })
    },
    updateItemData: (id, params) => dispatch(updateApply(id, params))
  }
}
MyForm = createForm({
  mapPropsToFields(props) {
    ...
    for (let prop in props.itemData) {
       result["apply." + prop] = {value: props.itemData[prop]}
        if (prop == "info"){
         for (let prop1 in props.itemData["info"]){
           result["apply.info_" + prop1] = {value : props.itemData['info'][prop1]}
         }
        }else if(prop == "level"){
          for (let prop1 in props.itemData["level"]){
            result["apply.level_" + prop1] = {value : props.itemData['level'][prop1]}
          }
        }else if(prop == "account"){
          for (let prop1 in props.itemData["account"]){
            result["apply.account_" + prop1] = {value : props.itemData['account'][prop1]}
          }
        }
    }
    return result;
...
let ApplyNew = React.createClass({
  getInitialState() {
