        ...
        const uploadProps = {
            ...
            onChange: (info)=> {
                ...
                if (info.file.status === 'done') {
                    ...
                    var filesData = this.state.fileList;
                    _.map(filesData, function (item) {
                        if (info.file.uid == item.uid) {
                            _.merge(item, {url: imageUrl, url_key: info.file.response.key})
                        }
                    })
                     //全部图片上传完毕处理,处理回调
                     if( _.every(filesData, ['status', 'done' ])){
                       if(this.props.onAllUploadComplete){
                         this.props.onAllUploadComplete(filesData)
                       }
                     };
                  // 验证数据
                    imgsData = filesData;
                    console.log(info , "ffffffff111111" )
                    this.setState({fileList: filesData})
                } else {
                    // 验证数据
