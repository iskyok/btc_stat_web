...
import ImageExploreControls from "./toolBar/image_explore"
...
class EditorConcist extends React.Component {
  constructor(props) {
    ...
    this.onChange = (editorState) => {
      console.log("33333333sssssssss",editorState)
      ...
      that.timer = setTimeout(function() {
        ...
        if(ConvertFormatProps === 'html') {
          ...
          const raw_content = convertToRaw(rawContentState);
          rawContent = JSON.stringify(raw_content);
          content = stateToHTML(rawContentState);
        }

        // 去除空行
        let newContent = content.split("<p ><br></p>");
        newContent.map((item,i) => {
          if(item == "\n" )newContent[i]="<p ><br></p>"
        })
        let allContent = newContent.join("<span></span>");
        that.props.cbReceiver(rawContent,allContent); //富文本编辑器在设置active是true时，不可使用forceUpdate，否则会造成无法选中文本的问题！
      }, 300);
    };
    ...
    return (
      <div className="RichEditor-root editorHidden" content={this.state.HTML} id="text-editor-container">
        <div>
          ...
          {/*{this.props.ImageExplore&&<ImageExploreControls uploadConfig={this.props.uploadConfig} receiveImage={this.addImage}/>}*/}
