  ...
  render() {
    let properties = this.props, that = this, uploadProps = {
      ...
      data: (file) => {//支持自定义保存文件名、扩展名支持
        ...
        var randKey = timestamp + Math.random();
        key = randKey + "." + PRO_COMMON.String.GetFileExtensionName(file.name)[0];
    ...
    return (
      <div>
        ...
        <div style={{margin: "10px 0 0"}}>
          <Input
            placeholder={`您还可以手动输入${{
              'image': '图片',
              'video': '视频',
              'audio': '音频'
            }[this.props.fileType]}资源地址${this.props.fileType == "image" ? "（仅支持七牛）" : ""}，输入完毕按回车键确认`}
        ...
        <span>{`${this.props.limit > 1
          ? "最多"
          : "只"}可以上传 ${this.props.limit} 个类型为 ${PRO_QINIU.supportMime[this.props.fileType].join("、")} 的 ${{
          'image': '图片',
          'video': '视频',
          'audio': '音频'
        }[this.props.fileType]} 文件。${
